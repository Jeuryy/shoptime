...
export async function POST(req: NextRequest){
    ...
    if (event.type === "checkout.session.completed") {
        ...
        try {
            ...
            return NextResponse.json(
                {
                    erorr: "Error creating order: ", error,
                },
                {status: 400}
            );
        }
...
async function createOrderInSanity(
    ...
    const {orderNumber, customerName, customerEmail, clerkUserId, address} = 
        ...
        const lineItemsWithProuct = await stripe.checkout.sessions.listLineItems(
            id, 
        ...
        const stockUpdates=[]
        for (const item of lineItemsWithProuct.data){
